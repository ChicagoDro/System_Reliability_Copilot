...
def get_selections(df: pd.DataFrame, filters: dict) -> list[SelectionLike]:
    return [
        SelectionLike(
            entity_type="run", 
            entity_id=r["run_id"], 
            label=f"{r['platform']} - {r['resource_name']} ({r['status']})"
        )
...
def get_chips(sel: SelectionLike, filters: dict) -> list[Chip]:
    return [
        ...
        Chip(
            "run:logs", 
            "📄 Analyze Logs", 
            f"Fetch the logs for run {sel.entity_id} and explain why it failed.", 
            group="Diagnose"
        ),
        Chip(
            "run:fix", 
            "🔧 Suggest Fix", 
            f"Based on the failure message, what is the runbook procedure to fix run {sel.entity_id}?", 
            group="Optimize"
        ),
    ]
